...
# set the controller-gen image version
CONTROLLER_GEN_VERSION := v0.6.2

.PHONY: gen.run
gen.run: gen.api gen.tapp gen.crd
...
gen.crd:
	@$(DOCKER) run -it --rm \
		...
		controller-gen \
		crd paths=/go/src/$(ROOT_PACKAGE)/pkg/apis/... output:crd:dir=/go/src/$(ROOT_PACKAGE)/deploy/crds output:stdout

.PHONY: gen.tapp
gen.tapp:
	@$(DOCKER) run -it --rm \
		-v $(ROOT_DIR)/staging/tkestack.io/tapp:/go/src/tkestack.io/tapp \
		-e EXT_PB_APIS=$(EXT_PB_APIS)\
	 	$(REGISTRY_PREFIX)/code-generator:$(CODE_GENERATOR_VERSION) \
	 	/root/code.sh \
	 	all \
	 	tkestack.io/tapp/pkg/client \
	 	tkestack.io/tapp/pkg/apis \
	 	tkestack.io/tapp/pkg/apis \
		"tappcontroller:v1"
